...
const Hero = () => {
  ...
  // New function to handle departure date
  const handleDepartureDate = (date) => {
    console.log("Selected Departure Date:", date);
    console.log("Formatted Departure Date:", date ? date.toLocaleDateString() : "No date selected");
    setFormData(prev => ({
      ...prev,
      departDate: date,
  ...
  // New function to handle return date
  const handleReturnDate = (date) => {
    console.log("Selected Return Date:", date);
    console.log("Formatted Return Date:", date ? date.toLocaleDateString() : "No date selected");
    setFormData(prev => ({
      ...prev,
      returnDate: date
    }));
  ...
  const handleSubmit = (e) => {
    ...
    const message = `New booking request:%0A
        ...
        To: ${formData.to}%0A
        Departure: ${formData.departDate ? formData.departDate.toLocaleDateString() : 'Not selected'}%0A
        ${
          formData.tripType === "roundTrip"
            ? `Return: ${formData.returnDate ? formData.returnDate.toLocaleDateString() : 'Not selected'}`
            : ""
        }%0A
        Trip Type: ${formData.tripType}`;
    const whatsappUrl = `https://wa.me/+971551341387?text=${encodeURIComponent(
      message
    )}`;
  ...
  useEffect(() => {
    ...
    textureLoader.load("https://i.ibb.co/gZRRfzN/globe-map.png", (texture) => {
  ...
  return (
    <section
      id="home"
      className="relative bg-gradient-to-b from-sky-950 to-sky-900 text-white"
    >
      <div
        id="globe-container"
        className="absolute top-0 left-0 w-full h-[500px] overflow-hidden opacity-70"
      />
      ...
      <div className="relative pt-20 pb-16 px-4">
        <div className="container mx-auto">
          ...
          >
            ...
            >
              Book Air Tickets with{" "}
              <span className="text-orange-500 font-bold">Al Hania Travel</span>{" "}
              for hassle-free changes and cancellations.
            </motion.p>
          </motion.div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                  ...
                  <div className="flex gap-4">
                    <motion.label
                      className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg cursor-pointer transition-all duration-300 ${
                ...
                <div className="grid grid-cols-2 md:grid-cols-2 gap-6">
                  <div className="relative w-full">
                    ...
                    <DatePicker
                      ...
                      className="w-1/2 min-w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 transition-all duration-300"
